...
impl NormalTransformation for Polar {
  fn new() -> Polar {
...
impl InverseCumulative {
  ...
  {                       
    ...
    t = InverseCumulative::cdf(u)-q;    /* error */
...
impl NormalTransformation for InverseCumulative {
  //#[allow(dead_code)]
  fn new() -> InverseCumulative {
    InverseCumulative{ }
  }
...
mod test {
  ...
  fn test_acklam() {
    ...
    let x = f.iter().map(|&fi| InverseCumulative::inv_cdf(fi)).collect::<Vec<f64>>();
    for i in 0..N {
      assert!((f[i] - InverseCumulative::cdf(x[i])).abs() < std::f64::EPSILON);
    }
